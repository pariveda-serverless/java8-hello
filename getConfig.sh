#!/bin/bash
aws cloudformation list-exports --query 'Exports[?Name==`java8-hello-security-'"$STAGE"'`].Value' --output text > rolearn.txt
aws cloudformation list-exports --query 'Exports[?Name==`java8-hello-security-secrets-'"$STAGE"'`].Value' --output text > s3.txt
